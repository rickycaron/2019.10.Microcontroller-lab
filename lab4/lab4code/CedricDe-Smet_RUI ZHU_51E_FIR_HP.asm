;***********************************************************
; File Header(high pass filter)
;***********************************************************
;theory: multiple negative result and positive result separetly and then subtract them
    list p=18F25k50, r=hex, n=0
    #include <p18F25k50.inc>
    
    config SDOMX = RC7
    
    nmull	    equ	0x00; negative multiplication low byte
    nmulh	    equ	0x01; negative multiplication high byte
    pmull	    equ 0x02; positive
    pmulh	    equ 0x03; positive
    fmull	    equ 0x04; 
    fmulh	    equ	0x05;final result
    adcresult	    equ 0x06; 
...
next
    ...
    bra		positivemul
negativemul
     movf    nmull,0	;nmull->(W)
    addwf   PRODL,0	; PRODL+mull(W)->(w)
    movwf   nmull,1	;move the result(w) to mull
    movf    nmulh,0	;move mulh to W
    addwfc  PRODH,0	;add (W)+PRODH+Carrier and store it in (W)
    movwf   nmulh	;mov the result back to mulh
    bra	    checkaddress     
positivemul    
;    movf    pmull,0	;
;    addwf   PRODL,0	; PRODL+mull(W)->(w)
;    movwf   pmull,1	;move the result(w) to mull
;    movf    pmulh,0	;move mulh to W
;    addwfc  PRODH,0	;add (W)+PRODH+Carrier and store it in (W)
;    movwf   pmulh	;mov the result back to mulh
    movff   PRODL, pmull
    movff   PRODH, pmulh
...
firend
    movf	nmull,0; numll->(W)
    SUBWF	pmull,1; pmull-numll(W)->(pmull)
    movf	nmulh,0;nmulh->(W)
    SUBWFB	pmulh,1; pmulh-nmulh-C->pmulh
    movff	pmull,fmull
    movff	pmulh,fmulh
    movf	fmulh,0	;take out the final result
    addlw	0x5E
...
init_FIR
    ...
    movlw   D'41'	; b(3)
    movwf   0x33
    movlw   D'206'	; b(4) is positive other coefficients are all minus
    movwf   0x34
    movlw   D'41'	; b(5)
    movwf   0x35
    movlw   D'21'	; b(6)
